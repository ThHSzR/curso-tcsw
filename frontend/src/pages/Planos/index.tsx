...
export function Planos() {
  ...
  const [form, setForm] = useState<Omit<Plano, 'id'>>({ nome: '', descricao: '', preco: 0, duracaoMeses: 1 });
  const [editId, setEditId] = useState<number | null>(null);
  const [loading, setLoading] = useState(true);

  const load = () => { setLoading(true); planoService.getAll().then(d => { setPlanos(d); setLoading(false); }); };
  useEffect(() => { load(); }, []);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId !== null) { await planoService.update(editId, form); }
    else { await planoService.create(form); }
    setForm({ nome: '', descricao: '', preco: 0, duracaoMeses: 1 }); setEditId(null); load();
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    await planoService.remove(id); load();
  };

  return (
    <div className="page-container">
      <div className="page-header"><h2>Planos</h2></div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="row g-3">
              ...
              <div className="col-md-3">
                ...
                <input type="number" min={1} className="form-control" value={form.duracaoMeses} onChange={e => setForm({ ...form, duracaoMeses: +e.target.value })} required />
              </div>
            </div>
            <div className="d-flex gap-2 mt-3">
              <button type="submit" className="btn btn-primary">{editId ? 'Salvar' : 'Adicionar'}</button>
              {editId && <button type="button" className="btn btn-secondary" onClick={() => { setForm({ nome: '', descricao: '', preco: 0, duracaoMeses: 1 }); setEditId(null); }}>Cancelar</button>}
            </div>
          </form>
        </div>
      </div>
        ...
        <div className="card">
          <div className="card-body p-0">
            <table className="table table-hover mb-0">
              <thead>
                <tr><th>#</th><th>Nome</th><th>Descrição</th><th>Preço</th><th>Duração</th><th style={{ width: 100 }}>Ações</th></tr>
              </thead>
              <tbody>
                {planos.map(p => (
                  <tr key={p.id}>
                ...
                {planos.length === 0 && <tr><td colSpan={6} className="text-center text-muted py-4">Nenhum plano cadastrado.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
